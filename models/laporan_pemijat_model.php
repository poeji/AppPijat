...
function get_pemijat_terlaris($date1, $date2){
	/*$query = mysql_query("SELECT a.pijat_id, a.pijat_harga, a.pijat_name, jumlah
								...
								LIMIT 1
							 ");*/
	$query = mysql_query("SELECT COUNT(t.`pemijat_id`) AS jumlah, p.`nama_pemijat`
FROM `transactions` t
LEFT JOIN `pemijat` p ON p.`pemijat_id` = t.`pemijat_id`
WHERE t.transaction_date >= '$date1 00:00:00' AND t.transaction_date <= '$date2 23:59:59'
GROUP BY t.`pijat_id`
ORDER BY jumlah DESC LIMIT 1");
	$result = mysql_fetch_array($query);
	$result = ($result['nama_pemijat']) ? $result['nama_pemijat'] : "-";
	//$result = 0;
	return $result;
}
