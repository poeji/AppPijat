...
DATE_FORMAT(k.`dateinsert`, '%Y-%m-%d %H:%i:%s') AS tglins, r.`infrastruktur_name`, b.`branch_name`
...
LEFT JOIN `branches` b ON b.`branch_id` = k.`branch_id`
...
<body class="skin-blue">
 <div class="header_fixed">
    <div class="morph-button morph-button-modal morph-button-modal-3 morph-button-fixed" style="color: #6B346A !important">
      <button class="blue_color_button hide-on-mobile" style="color: #6B346A !important"  type="button"  onClick="javascript: window.location.href = 'c-order.php'; ">BACK TO ORDER</button>
    </div><!-- morph-button -->
                      ...
                      <div class="box_payment">
                        <div class="payment_title">bayar bill</div>
                          <div class="box-body2 table-responsive">
                             <form action="c-order.php?page=save_payment" method="post" enctype="multipart/form-data" role="form">
                             <input type="hidden" name="page" value="save_payment">
                             <?php /*
                             ...
                             <input type="hidden" name="id" value="<?=$_GET['id']?>">
                               ...
                               <input id="i_member" type="hidden" name="i_member" value="<?= $_GET['member']?>">
                               <div class="col-md-8">
                                 <div class="row">
...
opacity: 0;" type="radio" name="i_tax" id="i_tax" value="5" onclick="tax()">
                               ...
                               <?php
                                  ...
                                  $tjual = 0;
                                  $qtemp = mysql_query("SELECT d.`transaction_detail_id`, i.*, d.transaction_detail_original_price
                            ...
                            WHERE d.transaction_id = '".$_GET['id']."' AND d.transaction_detail_status = 0");
                                    while ($dtemp = mysql_fetch_array($qtemp)) {
                                      $subtotal = $dtemp['item_harga_jual'] * $dtemp['transaction_detail_original_price'];
                                      $tjual += $subtotal;
                                    }
                                  
                                        $pijatharga1 = 0;
                                        $qaddpijat1 = mysql_query("SELECT j.`pijat_harga` FROM `transaction_pijat` p
                                                    LEFT JOIN pijat j ON j.`pijat_id` = p.`pijat_id`
                                                    WHERE p.transaction_id = '".$_GET['id']."'");
                                        while($daddpjt1 = mysql_fetch_array($qaddpijat1)) {
                                          $pijatharga1 += $daddpjt1['pijat_harga'];
                                        }
                                  $total_price = $tjual + $pijatharga1;
                                  //$total_price = $tjual + $row_item['pijat_harga'] + $pijatharga1;
                                  $sc = $total_price * 0.05;
                                  $tax = $total_price * 0.1;
                              ...
                              <div class="col-md-12" style="padding:0px;">
                                <div class="payment_group">
                                  <div class="calc_title">
                                    ...
                                    <input min="0" required type="text" name="i_payment" id="i_payment" class="form-control calc_nominal" value="<?php echo round($totalkedua); ?>" style="text-align:right; background-color: #6B346A" onChange="update_change()"/>
                              ...
                              <div class="payment_group">
                                ...
                                <table width="100%">
                                   <tr>
                                    <td colspan="5">
                                      <button type="submit" class="btn button_save_payment btn-block" style="color: #6B346A">
                                        <i class="fa fa-save"></i> Simpan
                                      </button>
                                     </td>
                                      <td colspan="5" align="right">
                                        <a href="c-order.php" class="btn button_close_payment"  style="color: #6B346A">
                          ...
                          <div class="col-md-4" style="padding-left:5px;padding-right: 5px;">
                              <div class="payment_group">
                                ...
                                <table width="100%">
                                  <tr>
                                    <td></td>
                                    <td width="50%"><?= $row_item['infrastruktur_name'] ?></td>
                                    <td></td>
                                    <td width="20%">Date :</td>
                                  ...
                                  <tr>
                                    <td></td>
                                    <td width="50%"><?= $row_item['branch_name'] ?></td>
                                    <td></td>
                                    <td>Time :</td>
                                 ...
                                 <table id="" class="" width="100%">
                                    <tbody>
                                        ...
                                        <tr>
                                          ...
                                          <td valign="top"><?= ucwords($row_item['pijat_name']) ?></td>
                                        ...
                                        */ ?>
                                        ...
                                        $no = 1;
                                        $pijatharga = 0;
                                        $qaddpijat = mysql_query("SELECT j.`pijat_name`, j.`pijat_harga` FROM `transaction_pijat` p
                                                    LEFT JOIN pijat j ON j.`pijat_id` = p.`pijat_id`
                                                    WHERE p.transaction_id = '".$_GET['id']."'");
                                        ...
                                        <tr>
                                          ...
                                          <td valign="top"><?= ucwords($daddpjt['pijat_name']) ?></td>
                                          <td align="right" valign="top"><?= number_format($daddpjt['pijat_harga']) ?></td>
                                        </tr>
                                        <?php $pijatharga += $daddpjt['pijat_harga']; } ?>
                                           <?php 
                                           $tjual = 0;
                                           
                                           $qtemp = mysql_query("SELECT d.`transaction_detail_id`, i.*, d.`transaction_detail_original_price`, d.transaction_detail_original_price
          FROM `transaction_tmp_details` d 
LEFT JOIN item i ON i.`item_id` = d.`transaction_detail_item_qty`
WHERE d.transaction_id = '".$_GET['id']."' AND d.transaction_detail_status = 0");
        while ($dtemp = mysql_fetch_array($qtemp)) {
          ...
          ?>
                                           <tr>
                                          <td width="10%" valign="top"><?=$no?></td>
                                          <td valign="top"><?=$dtemp['item_name']?> [<?=$dtemp['transaction_detail_original_price']?>]</td>
                                          <td align="right" valign="top"><?php print number_format($subtotal); ?></td>
                                        </tr>
                                        <?php $tjual += $subtotal;  $no++; } ?>
                                    </tbody>
                                  </table>
                                  <?php 
                                  $total_price = $tjual + $pijatharga;
                                  //$total_price = $tjual + $row_item['pijat_harga'] + $pijatharga;
                                  $sc = $total_price * 0.05;
                                  $tax = $total_price * 0.1;
                                  $totalkedua = $total_price + $sc + $tax;
                                  ?>
                                  <div style="border-bottom:1px #000 dotted; padding-bottom:5px; margin-bottom:5px;"></div>
                                    <table width="100%">
                                     <tfoot>
                                        ...
                                        <tr>
                                          <td colspan="5"><a href="c-order.php?page=print_bill&id=<?=$_GET['id']?>&st=temp" target="_blank" class="btn button_save_payment btn-block"  style="color: #6B346A"><i class="fa fa-print"></i>Print Bill</a> </td>
                                        </tr>
                  ...
                  <div class="col-md-3">
                    <div class="payment_widget_frame">
                      <div class="payment_widget_header" style="background-color: #6B346A">
                        <div style="margin-bottom:10px; font-size:20px;"><?= "Table ". $row_item['infrastruktur_name'] ?></div>
                        <div><?= $row_item['transaction_code'] ?></div>
                      </div>
                      <div class="payment_widget_content">
                        ...
                        <div class="row">
                          <div class="form-group">
                           <a href="c-order.php?page=addpijat&member=<?=$_GET['member']?>&id=<?=$_GET['member']?>" class="btn payment_widget_button">Tambah Pijat</a>
                          </div>
                        </div>
                        <div class="row">
                          <div class="form-group">
                           <a href="c-order.php?page=editkursi&member=<?=$_GET['member']?>&id=<?=$_GET['member']?>" class="btn payment_widget_button">Pindah Kursi</a>
                          </div>
                        </div>
                        <?php if($_SESSION['user_type_id'] == 5) { ?>
                            <div class="row">
                              <div class="form-group">
                               <a href="c-order.php?page=additem&member=<?=$_GET['member']?>&id=<?=$_GET['member']?>" class="btn payment_widget_button">Tambah/Edit Item</a>
                              </div>
                            </div>
                            <div class="row">
                              <div class="form-group">
                               <a href="c-order.php?page=editpijat&member=<?=$_GET['member']?>&id=<?=$_GET['member']?>" class="btn payment_widget_button">Edit Pijat</a>
                              </div>
                            </div>
                            <div class="row">
                              <div class="form-group">
                               <a href="c-order.php?page=cancelorder&id=<?=$qdetkursi['transaction_id']?>&kursi=<?=$rinf['ruangan_infrastruktur_id']?>&ruangan=<?=$ruanganid?>&branch=<?=$branchid?>&t=<?=$qdetkursi['t']?>&member=<?=$qdetkursi['member_id']?>" class="btn payment_widget_button" onclick="return confirm('Apakah anda yakin akan membatalkan order ini?')">Cancel Order</a>
                              </div>
                            </div>
                        <?php } ?>
                        <?php /*
                        <div class="row">
                          <div class="form-group">
                           <a href="c-order.php?page=dashboard&member=<?=$_GET['member']?>&id=<?=$_GET['member']?>" class="btn payment_widget_button">Edit Item</a>
                          </div>
                        </div>
                        */ ?>
                        <div class="row">
                          <div class="form-group">
                           <a href="c-order.php?page=dashboard&member=<?=$_GET['member']?>&id=<?=$_GET['member']?>" class="btn payment_widget_button">Dashboard</a>
                          </div>
                        </div>
                        <!-- <div class="form-group">
                          <div class="row">
