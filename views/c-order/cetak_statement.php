...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-top: 5;font-size:12px;">
	<tr>
		...
		?>
		<td align="right" ><strong>Cashier: <?= ucfirst($usr['user_name']) ?></strong></td>
	</tr>
	<tr>
		<td></td>
		<td align="right" ></td>
...
<table>
	<tr>
		...
		?>
		<td>Nama: <?= ucfirst($rmember['member_name'])?></td>
	</tr>
	<tr>
...
<?php $no = 1; ?>
                                                        ...
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda mempunyai atau pernah mempunyai masalah tekanan darah tinggi ?</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['asma']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda menderita asma?</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['inhaler']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Jika ya, apakah anda perlu menggunakan inhaler saat perawatan berlangsung?</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['leher']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda sedang mengalami masalah leher dan punggung?</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['kulit']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda sedang memiliki masalah kulit, luka, cedera, atau infeksi?</label>
                                                            <label class="hitam">Ya [<?=$row['kulit_jabarkan']?>]</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['selain_diatas']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda memiliki masalah kesehatan selain yang telah disebutkan di atas dan perlu terapis anda ketahui?</label>
                                                            <label class="hitam">Ya [<?=$row['selain_jabarkan']?>]</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['alergi']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda memiliki alergi atau bahan-bahan tertentu yang dapat bereaksi terhadap kulit anda?</label>
                                                            <label class="hitam">Ya [<?=$row['alergi_jabarkan']?>]</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['hamil']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda sedang hamil atau sedang merencanakan kehamilan?</label>
                                                            <label class="hitam">Ya  [<?=$row['usia_kandungan']?>]</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['kontak_lens']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda menggunakan kontak lens?</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['melepas_lens']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Jika ya, apakah anda perlu melepasnya sebelum perawatan wajah atau perawatan lainnya?</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['tekanan']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Bagaimana level tekanan pijatan yang anda inginkan saat perawatan?</label>
                                                        ...
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Apakah anda ingin mendapatkan penawaran spesial kami melalui email atau pesan SMS/WA?</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['jawaban']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Saya menyatakan bahwa jawaban yang berikan adalah benar</label>
                                                            <label class="hitam">Ya</label>
                                                        </div><?php } ?>

                                                        <?php if ($row['tidak_menyembunyikan']==1){ ?>
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Saya tidak menyembunyikan informasi apapun yang mungkin relevan untuk menentukan bagaimana
                                                        ...
                                                        <div id="tekanan">
                                                             <label><strong><?=$no++?>.</strong> Saya mengetahui bahwa Zee Holistic Living tidak bertanggung jawab atas cedera atau kerusakan
...
<a href="c-order.php?page=payment&member=<?=$_GET['member']?>&id=<?=$_GET['id']?>" style="text-decoration:none" title="Order"><div class="back_to_order">Order</div></a>

<?php /*
<a href="order.php?page=list&member=<?=$_GET['member']?>&id=<?=$_GET['id']?>&branch_id=<?=$_GET['branch_id']?>&ruangan_id=<?=$_GET['ruangan_id']?>&t=<?=$_GET['t']?>" style="text-decoration:none" title="Order" onclick="return confirm('Apakah anda yakin akan order ini?')"><div class="back_to_order">Edit Appointment</div></a>

onclick="return confirm('Apakah anda yakin akan order ini?')"
*/ ?>
