...
$qpjt = mysql_query("SELECT t.*, p.*, m.`member_name`, m.member_id, m.`member_alamat`, m.`member_phone`, 
DATE_FORMAT(k.`datebook`, '%d %M %Y %H:%i:%s') AS tgl, DATE_FORMAT(k.`dateinsert`, '%Y-%m-%d %H:%i:%s') AS tglins, r.`infrastruktur_name`
FROM `transactions_tmp` t LEFT JOIN pijat p ON p.`pijat_id` = t.`pijat` 
...
WHERE transaction_code = 0 AND ruangan_id = '".$_GET['ruangan_id']."' AND k.ruangan_infrastruktur_id = '".$_GET['ruangan_infrastruktur_id']."' AND DATE_FORMAT(t.`transaction_date`, '%Y%m%d') = '$_GET[t]'");
